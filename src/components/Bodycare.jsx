...
// ── Body care — a Monday–Sunday weekly schedule. Each entry is a body-care ritual
...
function Treatments() {
  ...
  const openNew = (wd) => setEditing({ weekday: wd, activity: blankActivity('protocol', { category: 'body', frequency: 'weekly', daysOfWeek: [wd], timeOfDay: ['morning'] }) })
        ...
        return (
          <section key={wd} className="border-t border-stone-200 pt-4">
            <div className="mb-3 flex items-center justify-between">
              ...
              <button onClick={() => openNew(wd)} className="text-sm text-stone-500 hover:text-stone-900">Add treatment</button>
            </div>
            {day.length === 0 ? (
                ...
                {day.map((a) => (
                  <div key={a.id} className="group flex items-start gap-3 border border-stone-200 bg-white/40 px-4 py-3">
                    <button onClick={() => openEdit(a)} className="min-w-0 flex-1 text-left">
                      <p className="font-serif text-lg text-stone-900">{a.title || 'Treatment'}</p>
                      {a.notes && a.notes.trim() && <p className="mt-1 whitespace-pre-line text-sm leading-relaxed text-stone-500">{a.notes}</p>}
                      <p className="kicker text-stone-400 mt-2">{PART_LABEL[itemPart(a)]}{isRecurring(a) ? ' · Weekly' : ' · One-time'}</p>
                    </button>
