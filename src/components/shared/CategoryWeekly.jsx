...
import { dateKey, parseKey, addDays, DOW_LONG, isSameDay } from '../../lib/date'
...
export default function CategoryWeekly({ category, noun = 'Item' }) {
  ...
  // Monday of the current week → the seven day dates.
      ...
      {days.map((d) => {
        ...
        return (
          <section key={k} className="border-t border-stone-200 pt-4">
            <h3 className={`font-serif italic text-2xl mb-3 ${isTod ? 'text-stone-900' : 'text-stone-800'}`}>{DOW_LONG[d.getDay()]}</h3>
            {dayItems.length === 0 ? (
