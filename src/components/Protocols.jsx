...
function TextArea({ label, value, onChange, placeholder }) {
  return (
    <div>
      <span className={labelCls}>{label}</span>
      <textarea value={value || ''} onChange={(e) => onChange(e.target.value)} placeholder={placeholder} className="w-full min-h-[90px] resize-y rounded-xl bg-stone-500/5 px-4 py-3 text-sm outline-none focus:border-stone-900" />
    </div>
  )
}
    ...
    <div>
      ...
      <div className="flex flex-wrap gap-1.5">
        {options.map((o) => {
          ...
          const on = value === id
          return (
            <button key={id} type="button" onClick={() => onChange(id)} className={`rounded-full px-3.5 py-1.5 text-xs border transition-colors ${on ? 'bg-stone-900 text-cream border-stone-900' : 'border-stone-300 text-stone-600 hover:border-stone-500'}`}>{lab}</button>
          )
        })}
      </div>
    </div>
      ...
      <div className="w-full max-w-xl bg-cream rounded-2xl border border-stone-200 shadow-2xl">
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-1.5">
              {PHASE_OPTS.map((o) => {
                ...
                return (
                  <button key={o.id} type="button" onClick={() => togglePhase(o.id)} className="flex items-center gap-1.5 rounded-full px-3.5 py-1.5 text-xs border transition-colors"
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-1.5">
              {PART_OPTS.map((o) => {
                ...
                return (
                  <button key={o.id} type="button" onClick={() => toggleTime(o.id)} className={`rounded-full px-3.5 py-1.5 text-xs border transition-colors ${on ? 'bg-stone-900 text-cream border-stone-900' : 'border-stone-300 text-stone-600 hover:border-stone-500'}`}>
                    {o.label}
                  </button>
                )
              })}
        ...
        <div className="flex items-center justify-between border-t border-stone-200 px-6 py-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={() => onSave({ ...draft, title: (draft.title || '').trim() || 'Untitled' })} className="rounded-full px-6 py-2 text-sm bg-stone-900 text-cream hover:bg-stone-700">Save</button>
          </div>
        </div>
      </div>
    </div>
