...
// ── Masks — a Monday–Sunday weekly schedule. Each mask is a Haircare ritual
// protocol (Morning → Morning Routine, Evening → Evening Routine on the home page).
function Masks() {
  const { activities, add, update, remove } = useActivities()
  const [editing, setEditing] = useState(null) // { weekday, activity }

  const masks = activities.filter((a) => a.type === 'protocol' && a.category === 'haircare' && a.status !== 'archived')
  const forDay = (wd) => masks.filter((a) => (isRecurring(a) ? (a.daysOfWeek || []).includes(wd) : a.seriesStart && parseKey(a.seriesStart).getDay() === wd))
        ...
        return (
          <section key={wd} className="border-t border-stone-200 pt-4">
            <div className="mb-3 flex items-center justify-between">
              ...
              <button onClick={() => openNew(wd)} className="text-sm text-stone-500 hover:text-stone-900">Add mask</button>
            </div>
            {items.length === 0 ? (
              <p className="text-sm italic text-stone-400">No mask.</p>
            ) : (
                ...
                {items.map((a) => (
                  <div key={a.id} className="group flex items-start gap-3 border border-stone-200 bg-white/40 px-4 py-3">
                    <button onClick={() => openEdit(a)} className="min-w-0 flex-1 text-left">
                      <p className="font-serif text-lg text-stone-900">{a.title || 'Mask'}</p>
                      {a.notes && a.notes.trim() && <p className="mt-1 whitespace-pre-line text-sm leading-relaxed text-stone-500">{a.notes}</p>}
                      <p className="kicker text-stone-400 mt-2">{PART_LABEL[maskPart(a)]}{isRecurring(a) ? ' · Weekly' : ' · One-time'}</p>
                    </button>
...
function MaskForm({ entry, isNew, onSave, onDelete, onClose }) {
  const { weekday } = entry
    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-stone-900/40 px-4 py-10 backdrop-blur-sm" onMouseDown={(e) => { if (e.target === e.currentTarget) onClose() }}>
      <div className="w-full max-w-lg bg-cream border border-stone-300 shadow-2xl">
        <div className="flex items-center justify-between border-b border-stone-200 px-6 py-5">
          <span className="kicker text-stone-400">{DOW_LONG[weekday]} mask</span>
        ...
        <div className="px-6 py-5 space-y-5">
          <div>
            ...
            <input
              ...
              placeholder="Hydrating hair mask"
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full min-h-[120px] resize-y bg-white/50 border border-stone-300 px-3 py-2 text-sm leading-relaxed outline-none focus:border-stone-900"
            />
          </div>
          <div>
